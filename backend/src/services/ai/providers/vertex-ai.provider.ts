...
import { ModelInfo } from '../../connections/connection.types';
...
export class VertexAIProvider {
  ...
  async generateContent(messages: VertexAIMessage[]): Promise<VertexAIResponse> {
  ...
  static convertToStandardMessage(vertexMessage: VertexAIMessage): { role: string; content: string } {
    return {
  ...
  static convertFromStandardMessage(standardMessage: { role: string; content: string }): VertexAIMessage {
    return {
