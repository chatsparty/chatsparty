...
import {
  FILE_SIZE_LIMITS,
} from '../services/storage/storage.validation';
...
export async function storageRoutes(fastify: FastifyInstance) {
   ...
   */
  fastify.post('/storage/projects/:projectId/upload', {
    preHandler: authenticate,
   ...
   * Download a file
   */
  fastify.get('/storage/download/:key', {
    preHandler: authenticate,
  }, async (request: FastifyRequest<{
    Params: { key: string };
   ...
   * Get file metadata
   */
  fastify.get('/storage/metadata/:key', {
    preHandler: authenticate,
  }, async (request: FastifyRequest<{
    Params: { key: string };
  }>, reply) => {
   ...
   * Get signed URL
   */
  fastify.post('/storage/signed-url', {
    preHandler: authenticate,
  }, async (request: FastifyRequest<{
    Body: {
      key: string;
    ...
    try {
      const url = await storageService.getSignedUrl(key, operation, expiresIn);
      const expiresAt = new Date(Date.now() + expiresIn * 1000);
      
      return { url, expiresAt };
    } catch (error: any) {
      fastify.log.error('Get signed URL error:', error);
      return reply.code(500).send({ message: 'Failed to generate signed URL' });
    }
  });
