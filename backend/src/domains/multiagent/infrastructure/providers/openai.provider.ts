...
const createOpenAIProvider = (
  ...
  const capabilities = PROVIDER_CAPABILITIES.openai;
  const errorHandler = handleProviderError('openai');
      ...
      retry(
        timeout(
          fromPromise(async () => {
            ...
            if (!response.object) {
              throw new Error('No object returned from OpenAI');
            }
...
export const openaiProviderFactory = createOpenAIProvider;