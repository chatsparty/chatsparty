...
const createGroqProvider = (
  ...
  const capabilities = PROVIDER_CAPABILITIES.groq;
  const errorHandler = handleProviderError('groq');
      ...
      retry(
        timeout(
          fromPromise(async () => {
            ...
            if (!response.object) {
              throw new Error('No object returned from Groq');
            }
...
export const groqProviderFactory = createGroqProvider;