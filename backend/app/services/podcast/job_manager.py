...
class PodcastJobManager:
    ...
    def create_job(self, session: Session, request: PodcastGenerationRequest, user_id: str) -> str:
    ...
    def get_job_status(self, session: Session, job_id: str, user_id: str) -> Optional[PodcastJobStatus]:
    ...
    def update_status(self, session: Session, job_id: str, status: str, error_message: Optional[str] = None):
        """Update job status."""
        stmt = select(PodcastJob).where(PodcastJob.id == job_id)
        result = session.execute(stmt)
        job = result.scalar_one_or_none()
        if job:
            job.status = status
    ...
    def update_progress(self, session: Session, job_id: str, processed_count: int):
        """Update job progress."""
        stmt = select(PodcastJob).where(PodcastJob.id == job_id)
        result = session.execute(stmt)
        job = result.scalar_one_or_none()
    ...
    def complete_job(self, session: Session, job_id: str, audio_path: str, duration: float, file_size: int):
