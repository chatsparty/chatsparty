...
export class WebSocketService {
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.ws.onclose = (event) => {
          ...
          // Handle authentication failures differently
          if (event.code === 4001) {
            console.error('[WEBSOCKET] 🔒 Authentication failed - token may be expired or invalid');
            console.error('[WEBSOCKET] Token used:', token.substring(0, 30) + '...');
            // Don't attempt to reconnect with same token
            return;
          }
  ...
  subscribe(channel: string): void {
    ...
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      ...
      console.log(`[WEBSOCKET] ⚠️ WebSocket not open, subscription queued for: ${channel}`);
    }
  }
  ...
  private handleMessage(data: string): void {
    try {
      ...
      if (handlers) {
        ...
        console.log(`[WEBSOCKET] ⚠️ No handlers found for message type: ${message.type}`);
      }
    } catch (error) {
