...
export const useConnections = () => {
  ...
  const fetchConnections = async () => {
    ...
    try {
      ...
      const [userConnectionsResponse, defaultConnectionResponse] = await Promise.all([
        axios.get('/connections'),
        axios.get('/default-connection').catch(() => ({ data: { enabled: false, connection: null } }))
      ]);
      
      // Handle user connections
      const userData = userConnectionsResponse.data?.data || userConnectionsResponse.data;
      ...
      if (defaultData.enabled && defaultData.connection) {
        const defaultConnection: ModelConnection = {
          ...
          description: defaultData.connection.description || 'System-provided default connection',
  ...
  const testConnection = async (id: string): Promise<ConnectionTestResult> => {
    try {
      ...
      if (connection?.is_system_default) {
        ...
        const response = await axios.post('/default-connection/test');
        return {
          success: response.data.success,
          message: response.data.message || 'System default connection test completed'
        };
      }
  ...
  const getActiveConnections = () => {
    return Array.isArray(connections) ? connections.filter(conn => conn.is_active) : [];
  };
