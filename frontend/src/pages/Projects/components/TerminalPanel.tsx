...
export const TerminalPanel: React.FC<TerminalPanelProps> = ({ projectId }) => {
  ...
  const [terminalInstances, setTerminalInstances] = useState<Map<string, any>>(
    new Map()
  );
  ...
  useEffect(() => {
    sessions.forEach((session) => {
      if (
        ...
        !terminalInstances.has(session.session_id)
      ) {
        ...
        if (container) {
          ...
          setTerminalInstances((prev) =>
            new Map(prev).set(session.session_id, terminal)
          );
        }
      }
    });
