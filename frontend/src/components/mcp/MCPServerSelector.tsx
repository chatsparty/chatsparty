...
const MCPServerSelector: React.FC<MCPServerSelectorProps> = ({
  ...
  const handleServerChange = (serverId: string) => {
    if (serverId === 'add-new') {
      navigate('/settings', { state: { activeTab: 'mcp-servers' } });
      return;
    }
  ...
  return (
    <div className="space-y-4">
            ...
            <AlertDescription>
              <div className="space-y-2">
                ...
                <Button 
                  variant="outline" 
                  size="sm" 
                  onClick={() => navigate('/settings', { state: { activeTab: 'mcp-servers' } })}
              ...
              >
                    ...
                    <div className="flex-1">
                      <CardTitle className="text-sm flex items-center gap-2">
                        ...
                        <Badge 
                          ...
                          className={server.status === 'active' ? 'bg-green-100 text-green-800 text-xs' : 'text-xs'}
                        >
                          {server.status}
                        </Badge>
                      </CardTitle>
                ...
                <CardContent className="pt-0">
                  <div className="text-xs text-muted-foreground">
                    <div>URL: <code className="bg-muted px-1 rounded">{server.server_url}</code></div>
                    <div className="mt-1">
            ...
            >
              <CardContent className="flex items-center justify-center py-6">
                <div className="text-center">
                  ...
                  <div className="text-sm font-medium text-muted-foreground">Add New MCP Server</div>
                  <div className="text-xs text-muted-foreground">Configure a new server with tools</div>
                </div>
              </CardContent>
            </Card>
          </div>
      ...
      {/* Tool Selection */}
      {selectedServer && selectedServer.available_tools && selectedServer.available_tools.length > 0 && (
        <div>
          ...
          {selectedTools.length > 0 && (
            <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <div className="text-sm text-blue-800">
                <strong>Selected tools:</strong> This agent will be able to use {selectedTools.length} tool{selectedTools.length !== 1 ? 's' : ''} 
      ...
      {selectedServer && (!selectedServer.available_tools || selectedServer.available_tools.length === 0) && (
        <Alert>
          <AlertCircle className="h-4 w-4" />
          <AlertDescription>
            This server doesn't have any discovered tools yet. The server may need to be tested or configured properly.
          </AlertDescription>
        </Alert>
      )}
    </div>
